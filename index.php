...
<!-- modal end -->
  ...
  <header id="header" class="fixed-top ">
    <div class="container d-flex align-items-center">
      ...
      <h1 class="logo mr-auto"><a href="#header" class="scrollto">Football Area</a></h1>
    ...
    <div id="heroCarousel" class="container carousel carousel-fade" data-ride="carousel">
      ...
      <div class="carousel-item active">
        <div class="carousel-container">
          <h2 class="animate__animated animate__fadeInDown">Team 1 1 : 2 Team 2</h2>
          <p class="animate__animated animate__fadeInUp">2 september 2021</p>
        </div>
      </div>
  ...
  <!-- <script>
    //pake ajax
    $.ajax({
    headers: { 'X-Auth-Token': '35d350eea8964c479a27b26f0559355f' },
  ...
  <script>
    ...
    const getListData = () => {
        ...
        .then(response => {
            let Data = response.data.childAreas
              // console.log(Data)
            let toHtml = ""
              Data.forEach(childAreas => {
                toHtml += `<div class="col-md-6 col-lg-3  mb-5 mb-lg-2">
                            <div class="card">
                              <div class="card-body">
                                <div class="icon"><i class="bx bxl-dribbble"></i></div>
                                <h4 class="title"><a data-toggle="modal" class="mdl" data-id="${childAreas.id}" data-target="#staticBackdrop" onclick="modal('${childAreas.name}')">${childAreas.name}</a></h4>
                                <p class="description">20 competitions</p>
                              </div>
                            </div>
                          </div>`
            });
    ...
    function modal(b){
      ...
      function country(){
          ...
          .then(respponse => {
            ...
            const data_comp = response.data.areas
            
            data_comp.forEach(country => {
              console.log(response)
              let id_country = `${country.parentAreaId}`
              html += `<h5>${country.name}</h5>`
              if(id_benua == `${parentAreaId}`){
